...
func NewWdRequest(port int) *WdRequest {
	...
	mu := mysqlutil.MysqlUtil{}
	err := mu.InitMySqlUtilDetail(MYSQLHOST, 3306, "spider_center", "spiderdb@wd", "spider", 1, 1)
	if err != nil {
		panic(err)
	}
	if port == 1 {
...
func NewWdRequestLAN(port int) *WdRequest {
	...
	header := make(map[string]string)
	mu := mysqlutil.MysqlUtil{}
	err := mu.InitMySqlUtilDetail(LANMYSQLHOST, 3306, "spider_center", "spiderdb@wd", "spider", 1, 1)
	if err != nil {
		panic(err)
	}
	if port == 1 {
